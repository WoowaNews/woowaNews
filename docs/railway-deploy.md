...
- 프론트엔드는 `frontend/dist`로 빌드된 뒤 Spring Boot static resource로 jar에 포함됩니다.
...
Railway는 루트의 `railway.json`을 기준으로 다음 순서로 빌드합니다.

```sh
cd frontend && npm ci && npm run build && cd ../backend && ./gradlew clean bootJar
```
